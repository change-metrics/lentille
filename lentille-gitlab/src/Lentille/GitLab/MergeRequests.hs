...
streamMergeRequests ::
  MonadIO m => GitLabGraphClient -> Text -> UTCTime -> Text -> Stream (Of Changes) m ()
streamMergeRequests client host untilDate project =
  streamFetch client untilDate mkArgs (transformResponse host) breakOnDate
  where
